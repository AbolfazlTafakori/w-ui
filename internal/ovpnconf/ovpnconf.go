...
func RenderProfile(iface *model.Interface) string {
	...
	// Left unnamed on purpose, unlike the server's. What the device is called
	// on a customer's phone is theirs, and naming it after our interface would
	// collide with whatever else they already have connected.
...
func RenderServer(iface *model.Interface, l Layout) (string, error) {
	...
	// The device is named after the interface, not left to the kernel.
	//
	// `dev tun` gets an auto-numbered tun0, tun1, and everything in the panel
	// that is keyed on the interface name then refers to a device that does not
	// exist. That is not theoretical: the shaper looked for a device called
	// ovpn443, found tun0, and logged a failure every two seconds while every
	// speed limit on that tunnel silently did nothing. With two OpenVPN
	// interfaces there is also no way to tell which tun is which.
	fmt.Fprintf(&b, "dev %s\n", iface.Name)
