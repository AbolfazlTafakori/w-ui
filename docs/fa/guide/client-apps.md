...
روی گوشی، فایل را از صفحهٔ سابسکریپشن دانلود کن (⤓ کنار دستگاه) — با نام `device-1.ovpn` می‌آید نه `.txt`، و مستقیم در OpenVPN Connect باز می‌شود. فایل `.ovpn` که پنل می‌سازد خودکفاست — CA، کلید `tls-crypt` و آدرس سرور داخلش هستند — پس با یک ضربه وارد می‌شود. **نام کاربری و رمز** دستگاه داخل خود فایل است، پس بدون پرسیدن چیزی وصل می‌شود؛ همان زوج را می‌شود در کلاینت‌های قدیمی (OpenVPN قبل از 2.5) دستی تایپ کرد. سرتیفیکیت جداگانه‌ای برای نصب نیست.
