...
::: warning توکن چه کاری نمی‌تواند بکند
توکن مال ماشین است. کاری که فقط مدیر می‌تواند، با `403 this needs a signed-in administrator, not an API token` جواب می‌گیرد: حساب و دو‌مرحله‌ای خودِ مدیر، صدور یا باطل کردن توکن، بکاپ، ثبت نود و اعتماد به آن، تنظیمات و قالب پنل، ریستارت. هر چیز دربارهٔ مشتری‌ها، دستگاه‌ها، تانل‌ها، هاست‌ها، outboundها و مسیریابی برای توکن باز است — توکن برای همین است.
:::
